...
export default async function SearchPage({ searchParams }: SearchPageProps) {
  ...
  const result = await searchListings(supabase, { ...query, page, pageSize }).catch((error) => {
    ...
    return { listings: [], page, pageSize, total: 0, totalPages: 1 };
  });
        ...
        <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="font-display text-xl font-semibold text-foreground">
              {query.location ? `Stays in ${query.location}` : "Explore stays"}
            </p>
          ...
          <div className="flex items-center gap-2">
            <Sheet>
              ...
              <SheetContent side="left" className="w-[92vw] overflow-y-auto p-0 sm:max-w-md">
                <SheetHeader className="border-b border-border p-5 text-left">
                  <SheetTitle>Filters</SheetTitle>
                </SheetHeader>
                <div className="p-5">
                  <Filters className="border-0 p-0 shadow-none" />
                </div>
              </SheetContent>
            </Sheet>
            <SortSelect />
            <Sheet>
              ...
              <SheetContent side="bottom" className="h-[100dvh] w-full max-w-none p-0">
                ...
                <ListingsMap
                  ...
                  className="h-[calc(100dvh-65px)] w-full rounded-none border-0"
                />
              </SheetContent>
            </Sheet>
          </div>
